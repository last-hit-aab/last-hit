...
import { Flow, saveFlow, Step, StepAssertion, StepAssertOperator, Story } from '../../workspace-settings';
...
export default (props: { open: boolean; story: Story; flow: Flow; step: Step; close: () => void }): JSX.Element => {
	...
	const onMoreAssertClicked = (): void => {
		remote.dialog.showMessageBox(remote.getCurrentWindow(), {
			type: 'info',
			title: 'Pro',
			message: '"More Assertions" is a coming soon feature in PRO version.'
		});
	};
	...
	return (
		<Dialog open={open} onClose={() => close()} fullWidth={true} disableBackdropClick={true}>
			<DialogTitle>Step Assertion</DialogTitle>
			<DialogContent>
				<DialogContentText>Please, specify step assertion.</DialogContentText>
				<TextField
					autoFocus
					label="DOM Element"
					margin="dense"
			...
			<DialogActions>
				...
				<Button onClick={onMoreAssertClicked} color="primary" variant="contained">
					More Assert
				</Button>
